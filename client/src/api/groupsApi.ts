...
export const groupsApi = {
  ...
  getDebts: async (groupId: Id): Promise<GroupDebt[]> => {
    ...
    try {
      ...
      if (isGroupDebtsNullError(error)) {
        console.warn(
          "Backend returned null for groupDebts. Showing an empty debts list.",
        );
        return [];
      }
  ...
  markDebtAsPaid: async (debtId: Id) => {
    ...
    `;
    const response = await graphqlClient<{ markDebtAsPaid: boolean }>(
      mutation,
      { debtId },
    );
  ...
  confirmDebtPayment: async (debtId: Id) => {
    ...
    `;
    const response = await graphqlClient<{ confirmDebtPayment: boolean }>(
      mutation,
      { debtId },
    );
